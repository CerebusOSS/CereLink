...
/// truth in cbhwlib.h (Central) and cbproto.h (cbProto).
///
///////////////////////////////////////////////////////////////////////////////////////////////////
...
constexpr uint32_t CENTRAL_cbVERSION_MAJOR = 4;
constexpr uint32_t CENTRAL_cbVERSION_MINOR = 1;
...
constexpr uint32_t CENTRAL_cbMAXPROCS = 1;          ///< Maximum supported NSPs
constexpr uint32_t CENTRAL_cbNUM_FE_CHANS = 256;    ///< Maximum supported front-end channels
constexpr uint32_t CENTRAL_cbMAXGROUPS = 8;         ///< Sample rate groups
constexpr uint32_t CENTRAL_cbMAXFILTS = 32;         ///< Digital filters
constexpr uint32_t CENTRAL_cbMAXVIDEOSOURCE = 1;    ///< Maximum number of video sources
constexpr uint32_t CENTRAL_cbMAXTRACKOBJ = 20;      ///< Maximum number of trackable objects
constexpr uint32_t CENTRAL_cbMAXHOOPS = 4;          ///< Maximum number of hoops for spike sorting
constexpr uint32_t CENTRAL_cbMAXSITES = 4;          ///< Maximum number of electrodes in an n-trode group
constexpr uint32_t CENTRAL_cbMAXSITEPLOTS = ((CENTRAL_cbMAXSITES - 1) * CENTRAL_cbMAXSITES / 2);  ///< Combination of 2 out of n
constexpr uint32_t CENTRAL_cbMAXUNITS = 5;          ///< Maximum number of sorted units per channel
constexpr uint32_t CENTRAL_cbMAX_PNTS = 128;        ///< Maximum spike waveform points
constexpr uint32_t CENTRAL_cbMAX_AOUT_TRIGGER = 5;  ///< Maximum number of per-channel (analog output, or digital output) triggers
...
constexpr uint32_t CENTRAL_cbLEN_STR_UNIT = 8;       ///< Length of unit string
constexpr uint32_t CENTRAL_cbLEN_STR_LABEL = 16;      ///< Length of label string
constexpr uint32_t CENTRAL_cbLEN_STR_FILT_LABEL = 16;      ///< Length of filter label string
constexpr uint32_t CENTRAL_cbLEN_STR_IDENT = 64;      ///< Length of identity string
constexpr uint32_t CENTRAL_cbLEN_STR_COMMENT = 256;     ///< Length of comment string
constexpr uint32_t CENTRAL_cbMAX_COMMENT = 128;     ///< Maximum comment length (must be multiple of 4)
...
static_assert(sizeof(cbPKT_HEADER) == 16);
...
static_assert(sizeof(cbPKT_SYSINFO) == 40);
...
typedef struct {
    ...
    char   ident[wire::CENTRAL_cbLEN_STR_IDENT];   ///< ID string with the equipment name of the Signal Processor
...
static_assert(sizeof(cbPKT_PROCINFO) == 128);
...
typedef struct {
    ...
    char   ident[wire::CENTRAL_cbLEN_STR_IDENT];   ///< ID string with the equipment name of the Signal Bank hardware module
    char   label[wire::CENTRAL_cbLEN_STR_LABEL];   ///< Label on the instrument for the signal bank, eg "Analog In"
...
static_assert(sizeof(cbPKT_BANKINFO) == 116);
...
typedef struct {
    ...
    char   label[wire::CENTRAL_cbLEN_STR_LABEL];  ///< sampling group label
    uint32_t period;     ///< sampling period for the group
    uint32_t length;     ///< number of channels in the list
    uint16_t list[wire::CENTRAL_cbNUM_ANALOG_CHANS];   ///< variable length list. The max size is the total number of analog channels
} cbPKT_GROUPINFO;
static_assert(sizeof(cbPKT_GROUPINFO) == 592);
...
typedef struct {
    ...
    uint32_t proc;       ///<
    uint32_t filt;       ///<
    char   label[wire::CENTRAL_cbLEN_STR_FILT_LABEL];  // name of the filter
...
static_assert(sizeof(cbPKT_FILTINFO) == 136);
...
static_assert(sizeof(cbPKT_ADAPTFILTINFO) == 36);
...
static_assert(sizeof(cbPKT_REFELECFILTINFO) == 28);
...
typedef struct {
    ...
    char    anaunit[wire::CENTRAL_cbLEN_STR_UNIT]; ///< the unit for the analog signal (eg, "uV" or "MPa")
} cbSCALING;
...
typedef struct {
    char    label[wire::CENTRAL_cbLEN_STR_FILT_LABEL];
    uint32_t  hpfreq;     ///< high-pass corner frequency in milliHertz
    uint32_t  hporder;    ///< high-pass filter order
    uint32_t  hptype;     ///< high-pass filter type
    uint32_t  lpfreq;     ///< low-pass frequency in milliHertz
...
typedef struct {
    ...
    char       label[wire::CENTRAL_cbLEN_STR_LABEL];   ///< Label of the channel (null terminated if <16 characters)
    ...
    cbMANUALUNITMAPPING unitmapping[wire::CENTRAL_cbMAXUNITS];            ///< manual unit mapping
    cbHOOP              spkhoops[wire::CENTRAL_cbMAXUNITS][wire::CENTRAL_cbMAXHOOPS];   ///< spike hoop sorting set
} cbPKT_CHANINFO;
static_assert(sizeof(cbPKT_CHANINFO) == 680);
...
typedef struct
{
    ...
    float  basis[wire::CENTRAL_cbMAX_PNTS][3];    ///< Room for all possible points collected
} cbPKT_FS_BASIS;
static_assert(sizeof(cbPKT_FS_BASIS) == 1564);
...
static_assert(sizeof(cbPKT_SS_MODELSET) == 100);
...
static_assert(sizeof(cbPKT_SS_DETECT) == 24);
...
static_assert(sizeof(cbPKT_SS_ARTIF_REJECT) == 24);
...
static_assert(sizeof(cbPKT_SS_NOISE_BOUNDARY) == 68);
...
static_assert(sizeof(cbPKT_SS_STATISTICS) == 64);
...
static_assert(sizeof(cbPKT_SS_STATUS) == 40);
...
typedef struct {
    ...
    char   label[wire::CENTRAL_cbLEN_STR_LABEL];   ///< Label of the Ntrode (null terminated if < 16 characters)
    cbMANUALUNITMAPPING ellipses[wire::CENTRAL_cbMAXSITEPLOTS][wire::CENTRAL_cbMAXUNITS];  ///< unit mapping
    uint16_t nSite;          ///< number channels in this NTrode ( 0 <= nSite <= cbMAXSITES)
    uint16_t fs;             ///< NTrode feature space cbNTRODEINFO_FS_*
    uint16_t nChan[wire::CENTRAL_cbMAXSITES];  ///< group of channels in this NTrode
} cbPKT_NTRODEINFO;
static_assert(sizeof(cbPKT_NTRODEINFO) == 1008);
...
typedef struct
{
    ...
    union {
        ...
        struct {
            ...
            uint16_t duration[CENTRAL_cbMAX_WAVEFORM_PHASES];     ///< array of durations for each phase
            int16_t  amplitude[CENTRAL_cbMAX_WAVEFORM_PHASES];    ///< array of amplitude for each phase
        };
    };
} cbWaveformData;
static_assert(sizeof(cbWaveformData) == 992);
...
static_assert(sizeof(cbPKT_LNC) == 28);
...
typedef struct {
    char    name[wire::CENTRAL_cbLEN_STR_LABEL];  ///< filename of the video file
...
typedef struct {
    char     name[wire::CENTRAL_cbLEN_STR_LABEL];  ///< name of the object
...
typedef struct {
    ...
    char   username[wire::CENTRAL_cbLEN_STR_COMMENT];     ///< name of computer issuing the packet
    union {
        char   filename[wire::CENTRAL_cbLEN_STR_COMMENT]; ///< filename to record to
        char   datetime[wire::CENTRAL_cbLEN_STR_COMMENT]; ///<
    };
    char   comment[wire::CENTRAL_cbLEN_STR_COMMENT];  ///< comment to include in the file
} cbPKT_FILECFG;
static_assert(sizeof(cbPKT_FILECFG) == 800);
...
typedef struct {
    ...
    int16_t  wave[wire::CENTRAL_cbMAX_PNTS];    ///< datapoints of each sample of the waveform. Room for all possible points collected
    ///< wave must be the last item in the structure because it can be variable length to a max of cbMAX_PNTS
} cbPKT_SPK;
static_assert(sizeof(cbPKT_SPK) == 288);
...
typedef struct
{
    ...
    uint16_t   abyUnitSelections[(CENTRAL_cbPKT_MAX_SIZE - CENTRAL_cbPKT_HEADER_SIZE - sizeof(int32_t))];     ///< one for each channel, channels are 0 based here, shows units selected
} cbPKT_UNIT_SELECTION;
static_assert(sizeof(cbPKT_UNIT_SELECTION) == 2028);

///////////////////////////////////////////////////////////////////////////////////////////////////
/// @name Central Constants
/// @{

constexpr uint32_t CENTRAL_cbMAXPROCS = 3;          ///< Maximum supported NSPs
constexpr uint32_t CENTRAL_cbNUM_FE_CHANS = 512;    ///< Maximum supported front-end channels
constexpr uint32_t CENTRAL_cbMAXGROUPS = 8;         ///< Sample rate groups
constexpr uint32_t CENTRAL_cbMAXFILTS = 32;         ///< Digital filters
constexpr uint32_t CENTRAL_cbMAXVIDEOSOURCE = 1;    ///< Maximum number of video sources
constexpr uint32_t CENTRAL_cbMAXTRACKOBJ = 20;      ///< Maximum number of trackable objects
...
constexpr uint32_t CENTRAL_cbMAXNTRODES = CENTRAL_cbNUM_FE_CHANS / 2;  ///< = 256

/// Analog output gain channels (Central's multi-instrument count)
constexpr uint32_t CENTRAL_AOUT_NUM_GAIN_CHANS = CENTRAL_cbNUM_ANAOUT_CHANS + CENTRAL_cbNUM_AUDOUT_CHANS;  ///< = 18
...
/// Receive buffer size
constexpr uint32_t CENTRAL_cbRECBUFFLEN = CENTRAL_cbNUM_FE_CHANS * 65536 * 4;
...
typedef struct {
    cbPKT_FS_BASIS          asBasis[CENTRAL_cbMAXCHANS];    ///< PCA basis values per channel
    cbPKT_SS_MODELSET       asSortModel[CENTRAL_cbMAXCHANS][CENTRAL_cbMAXUNITS + 2];    ///< Sorting models/rules per channel
    ...
    cbPKT_SS_NOISE_BOUNDARY pktNoiseBoundary[CENTRAL_cbMAXCHANS]; ///< Noise boundaries per channel
    cbPKT_SS_STATISTICS     pktStatistics;    ///< Spike statistics
    cbPKT_SS_STATUS         pktStatus;        ///< Spike sorting status
} cbSPIKE_SORTING;
static_assert(sizeof(cbSPIKE_SORTING) == 1390024);
...
struct cbCFGBUFF {
    ...
    cbPKT_AOUT_WAVEFORM isWaveform[CENTRAL_AOUT_NUM_GAIN_CHANS][CENTRAL_cbMAX_AOUT_TRIGGER];
    cbPKT_LNC         isLnc[CENTRAL_cbMAXPROCS];
    cbPKT_NPLAY       isNPlay;
    cbVIDEOSOURCE     isVideoSource[CENTRAL_cbMAXVIDEOSOURCE];
    cbTRACKOBJ        isTrackObj[CENTRAL_cbMAXTRACKOBJ];
    cbPKT_FILECFG     fileinfo;
    // hwndCentral omitted (at end, variable size, not needed by CereLink)
};
static_assert(sizeof(cbCFGBUFF) == 2183896);
...
static_assert(sizeof(cbXMTBUFF) == 290400028);
...
static_assert(sizeof(cbXMTBUFFLOCAL) == 116160028);
...
static_assert(sizeof(cbSPKCACHE) == 115220);
...
static_assert(sizeof(cbSPKBUFF) == 68671136);
...
static_assert(sizeof(APP_WORKSPACE) == 28);
...
struct cbPcStatus {
    ...
    APP_WORKSPACE m_icAppWorkspace[CENTRAL_cbMAXAPPWORKSPACES]; ///< App workspace config
};
static_assert(sizeof(cbPcStatus) == 6440);
...
static_assert(sizeof(cbRECBUFF) == 536870932);
