...
TEST_F(ShmemSessionTest, CreateStandalone) {
    auto result = ShmemSession::create(test_name, test_name + "_xmt", Mode::STANDALONE);
...
TEST_F(ShmemSessionTest, MoveConstruction) {
    auto result1 = ShmemSession::create(test_name, test_name + "_xmt", Mode::STANDALONE);
...
TEST_F(ShmemSessionTest, MoveAssignment) {
    auto result1 = ShmemSession::create(test_name + "_1", test_name + "_1_xmt", Mode::STANDALONE);
    auto result2 = ShmemSession::create(test_name + "_2", test_name + "_2_xmt", Mode::STANDALONE);
    ASSERT_TRUE(result1.isOk());
...
TEST_F(ShmemSessionTest, OperationsOnClosedSession) {
    ...
    {
        auto result = ShmemSession::create(name, name + "_xmt", Mode::STANDALONE);
        ASSERT_TRUE(result.isOk());
    }
    // Session is now closed

    // Try to create a new session with same name should work
    auto result2 = ShmemSession::create(name, name + "_xmt", Mode::STANDALONE);
    ASSERT_TRUE(result2.isOk());
}
